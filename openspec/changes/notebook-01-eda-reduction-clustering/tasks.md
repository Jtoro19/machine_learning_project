...
- [ ] 0.1 Verify the `ResultsWriter` artifact-name uniqueness assumption against committed
...
- [ ] 1.1 `s0_00_title` (md): Title, the three questions the notebook answers (data shape, feature
...
- [ ] 1.2 `s0_01_imports` (code): import `dataclasses`, `numpy`, `pandas`,
...
- [ ] 1.3 `s0_02_style` (code): `%matplotlib inline`; `sns.set_theme(style="whitegrid",
...
- [ ] 1.4 `s0_03_constants` (code): `NOTEBOOK_ID = "eda_reduction_clustering"`, `SEED = 42`,
...
- [ ] 1.5 `s0_04_data_guard_md` (md): why raw validation runs before anything else — a truncated or
  replaced CSV must fail at this cell, not halfway through section 3. Spec: Req13. Outcome: markdown
  renders.
- [ ] 1.6 `s0_05_validate` (code): `report = validate_raw_data()`; `print(report.render())`; `assert
  report.ok`. Spec: Req1, Req13. Outcome: assertion passes; report prints; cell has no `error` output.
- [ ] 1.7 `s0_06_load` (code): `cleaned = load_clean_partitions()` — called once, no arguments
...
- [ ] 1.8 `s0_07_boundary_md` (md): the `attack_cat` boundary — the four legitimate uses
...
- [ ] 1.9 `s0_08_assert_labels` (code) — **runtime assertion cell 1**:
  ```python
...
- [ ] 1.10 `s0_09_helpers` (code): define, in this order, `VariantSpace` (frozen dataclass: `key`,
...
- [ ] 1.11 `s0_10_spaces` (code): `pipelines = build_preprocessor_pair()`; for each `key, include_ttl
...
- [ ] 1.12 `s0_11_assert_matrix` (code) — **runtime assertion cell 2**:
  ```python
  ...
  confirmation line.
- [ ] 1.13 `s0_12_subsamples` (code) — the Decision 4 workaround: build
...
- [ ] 1.14 `s0_13_writer` (code): `writer = ResultsWriter(NOTEBOOK_ID)`; print `writer.directory`;
...
- [ ] 2.1 `s1_00_header` (md): what section 1 establishes; why it deliberately overlaps
...
- [ ] 2.2 `s1_01_shape` (code): column inventory — `position, column, dtype, non_null, role` — role
...
- [ ] 2.3 `s1_02_numeric_summary` (code): extended `describe` per numeric feature — `feature, count,
...
- [ ] 2.4 `s1_03_categorical` (code): long-format `value_counts` for `proto`, `service`, `state` —
...
- [ ] 2.5 `s1_04_attack_cat` (code): `attack_cat, rows, share`, computed at run time.
...
- [ ] 2.6 `s1_05_imbalance_md` (md): reading the class imbalance; why the bar chart uses a log
  x-axis (largest class outnumbers smallest by ~2 orders of magnitude). Spec: Req6. Outcome: markdown
  renders.
- [ ] 2.7 `s1_06_fig_attack_bars` (code): sorted horizontal bars (rows descending), log x-axis, one
...
- [ ] 2.8 `s1_07_skew_md` (md): right skew and the `log1p` rationale; states plainly that the ECDF
...
- [ ] 2.9 `s1_08_fig_hist_raw` (code): 2×3 histogram grid over `KEY_FEATURES`, `bins=50`, raw
  units, log y-axis. `add_figure(..., name="key_feature_histograms_raw", ...)`. Spec: Req6. Outcome:
  figure registered.
- [ ] 2.10 `s1_09_fig_hist_log1p` (code): same 2×3 grid after `log1p` **applied to a notebook-local
...
- [ ] 2.11 `s1_10_fig_ecdf` (code) — **R8, do not "fix" this back into two different curves**: 1×2
...
- [ ] 2.12 `s1_11_fig_boxplots` (code): 3×2 horizontal boxplot grid over `KEY_FEATURES`, grouped by
...
- [ ] 2.13 `s1_12_findings` (md): what section 1 establishes, in prose. Spec: Req6, Req11 (states
...
- [ ] 3.1 `s2_00_header` (md): scope of section 2; why the correlation matrices are single-variant
...
- [ ] 3.2 `s2_01_matrices` (code): `pearson_correlation_matrix` and `spearman_correlation_matrix`,
...
- [ ] 3.3 `s2_02_fig_pearson` (code): diverging heatmap, `cmap="RdBu_r"`, `vmin=-1`, `vmax=1`,
...
- [ ] 3.4 `s2_03_fig_spearman` (code): same palette/limits/geometry as 3.3. Colorbar label "Spearman
  rank correlation coefficient". `add_figure(..., name="spearman_correlation_heatmap", ...)`. Spec:
  Req7. Outcome: figure registered.
- [ ] 3.5 `s2_04_pairs` (code): upper-triangle pairs with `|r| > 0.9` under Pearson or Spearman —
...
- [ ] 3.6 `s2_05_vif_md` (md): VIF definition (`1/(1-R²)`), the three guards, why one-hot columns are
  excluded, why the modelled (post-`log1p`, pre-scaling-irrelevant) form is used. Spec: Req7. Outcome:
  markdown renders.
- [ ] 3.7 `s2_06_vif` (code): call `variance_inflation_factors(space.modelled)` for both variants,
  ...
  **string** `"inf"` when any feature in that variant has `vif == float("inf")`, and as the maximum
  ...
  string `"inf"`.
- [ ] 3.8 `s2_07_fig_vif` (code): grouped vertical bars, two bars per feature (variant pair colours
...
- [ ] 3.9 `s2_08_findings` (md): which feature families are redundant (e.g. `swin`/`dwin`, the
...
- [ ] 4.1 `s3_00_header` (md): what PCA is being asked; the design-matrix width per variant (~52
  columns, 36/34 modelled). Spec: Req8. Outcome: markdown renders.
- [ ] 4.2 `s3_01_fit` (code): for each variant, `pca = PCA(n_components=None, svd_solver="full",
  ...
  no error.
- [ ] 4.3 `s3_02_explained` (code): `variant, component, explained, cumulative`, both variants.
  `add_table(..., name="pca_explained_variance", sort_by=["variant", "component"])`. Spec: Req8,
  Req11. Outcome: table registered.
- [ ] 4.4 `s3_03_thresholds` (code): `variant, threshold, n_components` for thresholds `(0.90, 0.95)`.
...
- [ ] 4.5 `s3_04_fig_scree` (code): explained variance ratio per component, both variants overlaid,
  variant-pair colours and linestyles (`-`/`--`). `add_figure(..., name="pca_scree_plot", ...)`. Spec:
  Req8. Outcome: figure registered.
- [ ] 4.6 `s3_05_fig_cumulative` (code): cumulative curve, both variants, plus grey dashed horizontal
  reference lines at 0.90 and 0.95. `add_figure(..., name="pca_cumulative_variance", ...)`. Spec: Req8.
  Outcome: figure registered.
- [ ] 4.7 `s3_06_loadings` (code): top 15 loadings by `|value|` for PC1 and PC2, per variant —
  ...
  table registered.
- [ ] 4.8 `s3_07_fig_loadings` (code): 2×2 panel grid (variant × component {PC1, PC2}), horizontal
...
- [ ] 4.9 `s3_08_onehot_md` (md): F6 — the one-hot variance asymmetry; why it is measured, not fixed
  (`build_preprocessor`'s categorical branch has no scaler, out of scope to change). Spec: Req8.
  Outcome: markdown renders.
- [ ] 4.10 `s3_09_blocks` (code): the F6 measurement exactly as specified in design
...
- [ ] 4.11 `s3_10_fig_scatter` (code): 1×2 panels (one per variant), PC1 vs PC2 scores on
  ...
  `.fit(...)`.
- [ ] 4.12 `s3_11_findings` (md): how many real degrees of freedom exist; how the TTL toggle changed
...
- [ ] 5.1 `s4_00_header` (md): states the fit-on-full / evaluate-on-sample split before any number
...
- [ ] 5.2 `s4_01_sweep` (code): for each variant and `k in K_RANGE` (2..12), `kmeans_fits[(variant,
...
- [ ] 5.3 `s4_02_sweep_table` (code): `variant, k, inertia, silhouette, calinski_harabasz`, long
  ...
  and note registered.
- [ ] 5.4 `s4_03_fig_elbow` (code): inertia vs `k`, both variants overlaid, variant-pair colours.
  `add_figure(..., name="kmeans_elbow_inertia", ...)`. Spec: Req9. Outcome: figure registered.
- [ ] 5.5 `s4_04_fig_silhouette` (code): silhouette vs `k`, both variants overlaid, selected `k`
...
- [ ] 5.6 `s4_05_fig_ch` (code): Calinski-Harabasz vs `k`, both variants overlaid.
  `add_figure(..., name="kmeans_calinski_harabasz_by_k", ...)`. Spec: Req9. Outcome: figure registered.
- [ ] 5.7 `s4_06_select_k` (code): `selected_k[variant] = min(K_RANGE, key=lambda k: (-sweep[(variant,
...
- [ ] 5.8 `s4_07_dbscan_md` (md): why DBSCAN is sampled (ball-tree neighbour search degrades near ten
...
- [ ] 5.9 `s4_08_eps` (code): for each variant, `sample = scores[positions, :n90[variant]]`; `k =
...
- [ ] 5.10 `s4_09_fig_kdist` (code): sorted k-distance curves, both variants, each with a horizontal
...
- [ ] 5.11 `s4_10_dbscan` (code): for each variant, `DBSCAN(eps=dbscan_eps[v],
...
- [ ] 5.12 `s4_11_allocation` (code): `sub_default.to_frame()` and `sub_proportional.to_frame()`
  ...
  `floor=0`.
- [ ] 5.13 `s4_12_findings` (md): whether unsupervised structure exists; whether the two variants
...
- [ ] 6.1 `s5_00_header` (md): profiling is post hoc; no metric is optimised against `attack_cat`;
  ARI/NMI are explicitly out of scope (notebook 2's job). Spec: Req10. Outcome: markdown renders.
- [ ] 6.2 `s5_01_labels` (code): `final_labels[v] = kmeans_fits[(v, selected_k[v])].labels_` for both
...
- [ ] 6.3 `s5_02_profile` (code): per cluster, per **original, unscaled** numeric + skewed feature —
...
- [ ] 6.4 `s5_03_composition` (code): `attack_cat` composition per cluster, including zero cells —
...
- [ ] 6.5 `s5_04_findings` (md): what the clusters appear to represent, in original units. Spec:
...
- [ ] 7.1 `s6_00_header` (md): how to read section 6. Spec: Req11, Req13. Outcome: markdown renders.
- [ ] 7.2 `s6_01_summary` (code): assemble a compact decisive-metrics frame from `METRICS` (the
...
- [ ] 7.3 `s6_02_structure` (md, authored in the second pass): what structure exists, quoting the
...
- [ ] 7.4 `s6_03_clusters` (md, authored in the second pass): what the clusters appear to represent,
...
- [ ] 7.5 `s6_04_limitations` (md, authored in the second pass): subsample caveats (silhouette,
...
- [ ] 7.6 `s6_05_close` (code): `writer.write_json({"metrics": {k: v for k, (v, _) in
...
- [ ] 7.7 `s6_06_rerun` (md): the exact re-run command
  (`uv run jupyter nbconvert --to notebook --execute --inplace
...
- [ ] 8.1 Re-verify the five consumed `nids` signatures against **committed** source before writing
  ...
  updated.
- [ ] 8.2 Execute the notebook with the exact command:
  `uv run jupyter nbconvert --to notebook --execute --inplace
...
- [ ] 8.3 Verify `results/eda_reduction_clustering/manifest.json` exists, is valid JSON parseable by
...
- [ ] 8.4 Verify the two `attack_cat` boundary assertion cells (`s0_08_assert_labels`,
...
- [ ] 8.5 Run the full `uv run pytest` suite. It must stay green — this change adds no new tests
